<template>
    <div>
        ...
        <p class="mt-2 text-center text-sm text-gray-600">
            Or
            {{ ' ' }}
            <router-link :to="{ name: 'Login' }" class="font-medium text-indigo-600 hover:text-indigo-500">Log in now
            </router-link>
        </p>
    </div>
    <form class="mt-8 space-y-6" @submit="register">

        <div v-if="errorMsg" class="flex justify-between bg-red-500 text-white py-2 px-4 pr-0 rounded font-bold mb-4 shadow-lg">
            <div>
        ...
        <div class="rounded-md shadow-sm -space-y-px">
            <div>
                ...
                <input id="full-name" name="full-name" type="full-name" autocomplete="full-name" required=""
                    v-model="user.name"
